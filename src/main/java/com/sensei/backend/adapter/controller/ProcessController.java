...
public class ProcessController {
    ...
    @PostMapping
    public ResponseEntity<ProcessResponseDTO> create(@RequestBody ProcessRequestDTO dto) {
        return ResponseEntity.ok(processService.createProcess(dto));
    }

    @GetMapping("/by-activity/{activityId}")
    public ResponseEntity<List<ProcessResponseDTO>> getByActivity(@PathVariable UUID activityId) {
        return ResponseEntity.ok(processService.getByInteractiveActivity(activityId));
    }
}
